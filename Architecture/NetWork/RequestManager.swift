...
class RequestManager {
    ...
    /// 清除所有缓存
    func removeAllCache(completion: @escaping (Bool) -> Void) {
        CacheManager.default.removeAllCache(completion: completion)
    }

    /// 根据key值清除缓存
    func removeObjectCache(_ url: String, params: [String: Any]? = nil, dynamicParams: Parameters? = nil, completion: @escaping (Bool) -> Void) {
        let key = cacheKey(url, params, dynamicParams)
...
public class RequestTaskManager {
    ...
    /// 获取缓存Data
    @discardableResult
    public func cacheData(completion: @escaping (Data) -> Void) -> NetDataResponse {
        let dataResponse = NetDataResponse(dataRequest: dataRequest!, cache: cache, cacheKey: cacheKey, completionClosure: completionClosure)
        return dataResponse.cacheData(completion: completion)
    }

    /// 响应Data
    public func responseData(completion: @escaping (NetWorkValue<Data>) -> Void) {
        let dataResponse = NetDataResponse(dataRequest: dataRequest!, cache: cache, cacheKey: cacheKey, completionClosure: completionClosure)
        dataResponse.responseData(completion: completion)
    }

    /// 先获取Data缓存，再响应Data
    public func responseCacheAndData(completion: @escaping (NetWorkValue<Data>) -> Void) {
        let dataResponse = NetDataResponse(dataRequest: dataRequest!, cache: cache, cacheKey: cacheKey, completionClosure: completionClosure)
    ...
    /// 获取缓存String
    @discardableResult
    public func cacheString(completion: @escaping (String) -> Void) -> NetStringResponse {
        let stringResponse = NetStringResponse(dataRequest: dataRequest!, cache: cache, cacheKey: cacheKey, completionClosure: completionClosure)
        return stringResponse.cacheString(completion: completion)
    }

    /// 响应String
    public func responseString(completion: @escaping (NetWorkValue<String>) -> Void) {
        let stringResponse = NetStringResponse(dataRequest: dataRequest!, cache: cache, cacheKey: cacheKey, completionClosure: completionClosure)
        stringResponse.responseString(completion: completion)
    }

    /// 先获取缓存String,再响应String
    public func responseCacheAndString(completion: @escaping (NetWorkValue<String>) -> Void) {
        let stringResponse = NetStringResponse(dataRequest: dataRequest!, cache: cache, cacheKey: cacheKey, completionClosure: completionClosure)
    ...
    /// 获取缓存JSON
    @discardableResult
    public func cacheJson(completion: @escaping (Any) -> Void) -> NetJsonResponse {
        let jsonResponse = NetJsonResponse(dataRequest: dataRequest!, cache: cache, cacheKey: cacheKey, completionClosure: completionClosure)
        return jsonResponse.cacheJson(completion: completion)
    }

    /// 响应JSON
    public func responseJson(completion: @escaping (NetWorkValue<Any>) -> Void) {
        let jsonResponse = NetJsonResponse(dataRequest: dataRequest!, cache: cache, cacheKey: cacheKey, completionClosure: completionClosure)
        jsonResponse.responseJson(completion: completion)
    }

    /// 先获取缓存JSON，再响应JSON
    public func responseCacheAndJson(completion: @escaping (NetWorkValue<Any>) -> Void) {
        let jsonResponse = NetJsonResponse(dataRequest: dataRequest!, cache: cache, cacheKey: cacheKey, completionClosure: completionClosure)
        jsonResponse.responseCacheAndJson(completion: completion)
    }
}

// MARK: - NetBaseResponse
...
public class NetResponse {
    ...
    fileprivate var cacheKey: String
    fileprivate var completionClosure: (() -> Void)?
    fileprivate init(dataRequest: DataRequest, cache: Bool, cacheKey: String, completionClosure: (() -> Void)?) {
    ...
    ///
    fileprivate func response<T>(response: AFDataResponse<T>, completion: @escaping (NetWorkValue<T>) -> Void) {
    ...
    /// isCacheData
    fileprivate func responseCache<T>(response: AFDataResponse<T>, completion: @escaping (NetWorkValue<T>) -> Void) {
        if completionClosure != nil { completionClosure!() }
        let result = NetWorkValue(isCacheData: false, result: response.result, response: response.response)
        if NetWork.openUrlLog {
            NetWorkLog(response.request?.url?.absoluteString ?? "")
        }
        ...
        case let .failure(error):
            if NetWork.openResultLog {
                NetWorkLog(error.localizedDescription)
            }
        }
        completion(result)
    }
...
// MARK: - NetJsonResponse

public class NetJsonResponse: NetResponse {
    /// 响应JSON
    func responseJson(completion: @escaping (NetWorkValue<Any>) -> Void) {
        dataRequest.responseJSON(completionHandler: { response in
            self.response(response: response, completion: completion)
        })
    }

    fileprivate func responseCacheAndJson(completion: @escaping (NetWorkValue<Any>) -> Void) {
        if cache { cacheJson(completion: { json in
            let res = NetWorkValue(isCacheData: true, result: Alamofire.AFResult.success(json), response: nil)
            completion(res)
        }) }
    ...
    /// 获取缓存json
    @discardableResult
    fileprivate func cacheJson(completion: @escaping (Any) -> Void) -> NetJsonResponse {
...
// MARK: - NetStringResponse

public class NetStringResponse: NetResponse {
    /// 响应String
    func responseString(completion: @escaping (NetWorkValue<String>) -> Void) {
    ...
    @discardableResult
    fileprivate func cacheString(completion: @escaping (String) -> Void) -> NetStringResponse {
        if let data = CacheManager.default.objectSync(forKey: cacheKey)?.data,
           let str = String(data: data, encoding: .utf8)
        {
            completion(str)
    ...
    fileprivate func responseCacheAndString(completion: @escaping (NetWorkValue<String>) -> Void) {
        if cache { cacheString(completion: { str in
            let res = NetWorkValue(isCacheData: true, result: Alamofire.AFResult.success(str), response: nil)
            completion(res)
        }) }
...
// MARK: - NetDataResponse

public class NetDataResponse: NetResponse {
    /// 响应Data
    func responseData(completion: @escaping (NetWorkValue<Data>) -> Void) {
    ...
    @discardableResult
    fileprivate func cacheData(completion: @escaping (Data) -> Void) -> NetDataResponse {
        if let data = CacheManager.default.objectSync(forKey: cacheKey)?.data {
            completion(data)
        } else {
            if NetWork.openResultLog {
    ...
    fileprivate func responseCacheAndData(completion: @escaping (NetWorkValue<Data>) -> Void) {
        if cache { cacheData(completion: { data in
            let res = NetWorkValue(isCacheData: true, result: Alamofire.AFResult.success(data), response: nil)
            completion(res)
        }) }
