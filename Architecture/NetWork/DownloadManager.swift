...
class DownloadManager {
    ...
    func cancel(_ url: String, parameters: Parameters?, dynamicParams: Parameters? = nil) {
        ...
        NotificationCenter.default.post(name: NSNotification.Name("NetDownloadCancel"), object: nil)
    }
    ...
    /// 删除单个下载
    func delete(_ url: String, parameters: Parameters?, dynamicParams: Parameters? = nil, completion: @escaping (Bool) -> Void) {
        let key = cacheKey(url, parameters, dynamicParams)
        if let task = downloadTasks[key] {
            ...
            if let path = getFilePath(key) { /// 下载完成了
                do {
    ...
    /// 下载进度
    @discardableResult
    func downloadProgress(_ url: String, parameters: Parameters?, dynamicParams: Parameters? = nil, progress: @escaping ((Double) -> Void)) -> DownloadTaskManager? {
        let key = cacheKey(url, parameters, dynamicParams)
...
public class DownloadTaskManager {
    ...
    fileprivate var cancelCompletion: (() -> Void)?
    fileprivate var cccompletion: (() -> Void)?
         ...
         dynamicParams: Parameters? = nil)
    {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(downloadCancel), name: NSNotification.Name("NetDownloadCancel"), object: nil)
    ...
    /// 响应
    public func response(completion: @escaping (Alamofire.AFResult<String>) -> Void) {
...
func getDictionary(_ url: String) -> [String: Data] {
